...
def get_context(srcSents):
    srcSents = srcSents.data.cpu().numpy().tolist()
    contextSrcSents = []
    maxCtxSentLen = 0
    for srcSent in srcSents:
        # print(originSentPair)    ##PAD = 0   EOS = 1   BOS = 2   UNK = 3
        srcSent = [ x for x in srcSent if x not in [PAD, EOS, BOS] ]
        currentContext = sent2ctx[str(srcSent)].pop()
        if sent2ctx[str(srcSent)].__len__() == 0:
            sent2ctx.pop(str(srcSent))   ###every key will only be accessed 1 time, so delete after accessed
        currentContextAsSeq = []
        for sent in currentContext:
            currentContextAsSeq += sent
        contextSrcSents.append(currentContextAsSeq)
        maxCtxSentLen = max(maxCtxSentLen, currentContextAsSeq.__len__())
    ##BOS EOS PAD
